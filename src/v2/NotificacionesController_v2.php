...
// Enviar notificaciones push de dieta
function enviarNotificacionPushDieta(
    int $idNotificacion, 
    string $fecha,
    int $tipoIndicacion, 
    string $cama, 
    ...
    int $idDestinatario,
    string $dietaIndicacion,
    string $dietaCantidad,
    string $dietaFrecuencia,
    string $dietaDuracion,
    string $dietaObservaciones,
    string $profesional
    ): ?bool {
    ...
    if ($accessToken) {
        ...
        $notificationBody = 'Cama: '.$cama.' - Paciente: '.$paciente;

        // datos que se enviaran como datos personalizados en la notificación
        $extraData = [
            'idNotificacion' => (string)$idNotificacion,
            'tituloNotificacion' => (string)$notificationTitle,
            'fecha' => (string)$fecha,
            'tipoIndicacion' => (string)$tipoIndicacion,
            'cama' => $cama,
            ...
            'idDestinatario' => (string)$idDestinatario,
            'dietaIndicacion' => (string)$dietaIndicacion,
            'dietaCantidad' => (string) $dietaCantidad,
            'dietaFrecuencia' => (string) $dietaFrecuencia,
            'dietaDuracion' => (string) $dietaDuracion,
            'dietaObservaciones' => (string) $dietaObservaciones,
            'profesional' => (string) $profesional
        ];
    ...
    //$resultadoEnvio =  grabarEnvioNotificacion($idNotificacion, $idDestinatario, $idDispositivo, $statusCode, $mensaje, $dniEnfermero, $nombreEnfermero);
    
    $con = 'exec notificacionEnvio_log 
                @idNotificacion = :idNotificacion, 
                @idDestinatario = :idDestinatario,
                @idDispositivo = :idDispositivo,
                @statusCodeEnvio = :statusCodeEnvio,
                @respuestaFcm = :respuestaFcm,
                @dniEnfermero = :dniEnfermero,
                @nombreEnfermero = :nombreEnfermero';  
    ...
    $stmt->bindParam(":statusCodeEnvio", $statusCode);
    $stmt->bindParam(":respuestaFcm", $mensaje);
    $stmt->bindParam("dniEnfermero", $dniEnfermero);
    $stmt->bindParam("nombreEnfermero", $nombreEnfermero);
    $stmt->execute();
...
class NotificacionesController_v2
{
    // VER NOTIFICACIONES
    public function verNotificaciones(Request $request, Response $response, $args){
        ...
        if(isset($tokenAcceso[0])){
            if(verificarToken($tokenAcceso[0]) === true){
                ...
                if($error == 0){
                    ...
                    $con = 'EXEC notificacionesVer @dni = :dni, @idAplicacion = :idAplicacion';
                    try {
                        $db = getConeccionCAB();
                        $stmt = $db->prepare($con);
                        ...
                        foreach($res as $notificacion){
                            ...
                            $n->tipoNotificacion    = $notificacion->tipoNotificacion;
                            $n->tipoNotificacion    = (int)$notificacion->tipoIndicacion;
                            
                            // revisar este switch
                            switch ($notificacion->tipoIndicacion) {
                            ...
                            $n->dniPaciente         = $notificacion->dniPaciente;
                            $n->paciente            = $notificacion->apellidoPaciente . ', '.$notificacion->nombrePaciente;
                            $n->fechaIndicacion     = date_format(date_create($notificacion->fechaIndicacion), 'd-m-Y H:i:s');
                            ...
                            // medicacion
                            $n->medicacionGenerico   = $notificacion->medicacionGenerico;
                            $n->medicacionDosis      = $notificacion->medicacionDosis;
                            $n->medicacionVia        = $notificacion->medicacionVia;
                            $n->medicacionFrecuencia = $notificacion->medicacionFrecuencia;

                            //$n->medicacionCompuestos = $notificacion->medicacionCompuestos;
                            
                            if(!empty($notificacion->medicacionCompuestos)){
                                $n->medicacionCompuestos = json_decode($notificacion->medicacionCompuestos);
                            }else{
                                $n->medicacionCompuestos = [];
                            }

                            // dieta
                            $n->dietaIndicacion     = $notificacion->dietaIndicacion;
                            $n->dietaCantidad       = $notificacion->dietaCantidad;
                            $n->dietaFrecuencia     = $notificacion->dietaFrecuencia;
                            $n->dietaDuracion       = $notificacion->dietaDuracion;
                            $n->observaciones       = $notificacion->dietaObservaciones;

                            if(!empty($notificacion->leida)){
                                //$n->leida = json_decode($notificacion->leida);  //json
                                $n->leida = 1;
                            }else{
                                $n->leida = 0;
                            }
    ...
    public function verUnaNotificacion(Request $request, Response $response, $args){
        $tokenAcceso    = $request->getHeader('TokenAcceso');
        $idNotificacion = $args['idNotificacion'] ?? null;         
        ...
        if(isset($tokenAcceso[0])){
            if(verificarToken($tokenAcceso[0]) === true){
                ...
                if($error == 0){
                    // busco las notificaciones de este servicio
                    $con = 'EXEC notificacionesVerUna @idNotificacion = :idNotificacion';
                    try {
                        $db = getConeccionCAB();
                        $stmt = $db->prepare($con);
                        $stmt->bindParam(":idNotificacion", $idNotificacion);
                        ...
                        foreach($res as $notificacion){
                            ...
                            $n->idNotificacion      = (int)$notificacion->idNotificacion;
                            $n->tipoNotificacion    = $notificacion->tipoNotificacion;
                            $n->tipoIndicacion      = (int)$notificacion->tipoIndicacion;
                            
                            if($notificacion->tipoNotificacion == 'dieta'){
                                switch ($notificacion->tipoIndicacion) {
                                    case 1: $n->tituloNotificacion = 'NUEVA INDICACIÓN DE DIETA'; break;
                                    case 2: $n->tituloNotificacion = 'MODIFICACIÓN DE INDICACIÓN DE DIETA'; break;
                                    case 3: $n->tituloNotificacion = 'ELIMINACIÓN DE INDICACIÓN DE DIETA'; break;
                                    case 4: $n->tituloNotificacion = 'SUSPENCIÓN DE INDICACIÓN DE DIETA'; break;
                                }
                            }
                            
                            if($notificacion->tipoNotificacion == 'indicacionMedica'){
                                switch ($notificacion->tipoIndicacion) {
                                    case 1: $n->tituloNotificacion = 'NUEVA INDICACIÓN DE MEDICACIÓN'; break;
                                    case 2: $n->tituloNotificacion = 'MODIFICACIÓN DE INDICACIÓN DE MEDICACIÓN'; break;
                                    case 3: $n->tituloNotificacion = 'ELIMINACIÓN DE INDICACIÓN DE MEDICACIÓN'; break;
                                    case 4: $n->tituloNotificacion = 'SUSPENCIÓN DE INDICACIÓN DE MEDICACIÓN'; break;
                                }
                            }
                            ...
                            if(!empty($notificacion->leida)){
                                $n->leida = 1;
                                $n->leidaDetalle = json_decode($notificacion->leida);  //json
                            }else{
                                $n->leida = 0;
                                $n->leidaDetalle = [];
                            }
    ...
    public function marcarNotificacionLeida(Request $request, Response $response, $args){
        $tokenAcceso    = $request->getHeader('TokenAcceso');
        $json           = $request->getBody();
        $notificacion   = json_decode($json);

        $idNotificacion = $notificacion->idNotificacion ?? null;
        // $dni            = $notificacion->dni ?? null;
        // $nombre         = $notificacion->nombre ?? null;
        // $apellido       = $notificacion->apellido ?? null;  
        $idAndroid      = $notificacion->idAndroid ?? null;
        $idAplicacion   = $notificacion->idAplicacion ?? null;
            ...
            if(verificarToken($tokenAcceso[0]) === true){
                ...
                if($error == 0){
                    ...
                    $sql = 'declare @return_value int, @mensaje varchar(255)
                            EXEC @return_value = notificacionMarcarLeida
                                @idNotificacion = :idNotificacion,
                                @idAndroid = :idAndroid,
                    ...
                    try {
                        ...
                        $notif = $resultado[0];
                        $datos = array();
                        $datos = array('estado' => $notif->estado, 'mensaje' => $notif->mensaje);
    ...
    public function enviarNotificacion(Request $request, Response $response, $args){
        ...
        if(isset($datosNotif->tipoNotificacion) && $datosNotif->tipoNotificacion == 'dieta'){
            ...
            // datos del enfermero (no obligatorios)
            $datosEnf = $datosNotif->datos->enfermero ?? null;
            $dniEnfermero = $datosEnf->dniEnfermero ?? '';
            $nombreEnfermero = $datosEnf->nombreEnfermero ?? '';
            ...
            // datos de indicación de dieta


            if (empty($datosInd->fecha)){
                $error++;
            }else{
                // valido formato de fecha
                $fechaIndicacionDieta = '';
                try {
                    $dt = new \DateTime($datosInd->fecha);
                    $fechaIndicacionDieta = $dt->format('Ymd H:i:s');
                } catch (\Exception $e) {
                    $error++;
                }
            }
            ...
            if (isset($datosDie) && !empty($datosDie)) {
                ...
                // datos no obligatorios: cantidad, frecuencia, duración, observaciones
            }else{
                $error++;
            }
        }
            ...
            if(verificarToken($tokenAcceso[0]) === true){
                ...
                if($error == 0){
                    ...
                    if($tipoNotificacion == 'dieta'){
                        ...
                        $stmt->bindParam(":fechaIndicacion", $fechaIndicacionDieta);
                        ...
                        // 1- Obtengo los token de todos los dispositivos a los que hay que enviar la notificación. Puede ser solo cocina o cocina + enfermero.
                        // 2- Envió la notificación a los dispositivos de los cuales obtuve el token.
                        // 3- Armo la respuesta para Markey


                        // 1- OBTENCIÓN DE TOKENS
                    
                        $sql = 'EXEC obtenerTokensDispositivos @tipoNotificacion = :tipoNotificacion, @dniEnfermero = :dniEnfermero';
                        $db = getConeccionCAB();
                        $stmt = $db->prepare($sql);
                        $stmt->bindParam(":tipoNotificacion", $tipoNotificacion);
                        $stmt->bindParam(":dniEnfermero", $datosEnf->dniEnfermero);
                        $stmt->execute();
                        $resultado = $stmt->fetchAll(\PDO::FETCH_OBJ);
                        $db = null;
                        ...
                        foreach($resultado as $res){
                            // 2- ENVÍO DE LA NOTIFICACIÓN A CADA DISPOSITIVO
                            $tokenFcm = $res->tokenFcm;
                            $tipoIndicacion = $datosInd->tipoIndicacion;
                            $cama = $datosInt->cama;
                            $idDispositivo = $res->idDispositivo;
                            if($res->idAplicacion == 1){
                                $idDestinatario = 1; // Enfermería
                            }
                            if($res->idAplicacion == 3){
                                $idDestinatario = 2; // Cocina
                            }
                            

                            // envia la notificaión al dispositivo y graba el log en notificacionesEnvio
                            $paciente = $datosInt->apellidoPaciente . ', '.$datosInt->nombrePaciente;
                            $dniPaciente = $datosInt->dniPaciente;
                            $urgente = $datosInd->urgente;

                            
                            // observaciones
                            if (empty($datosDie->observaciones)) {
                                $datosDie->observaciones = '';
                            }

                            

                            if (enviarNotificacionPushDieta(
                                    $idNotificacion, 
                                    $fechaIndicacionDieta,
                                    $tipoIndicacion, 
                                    $cama, 
                        ...
                        // 3- ARMO LA RESPUESTA PARA MARKEY
                        if($resultadoEnvio > 0){
                            // si se envió a al menos un dispositivo, la notificación se considerará enviada correctamente
                            $datos = array(
                                'estado' => 1, 
                            ...
                            $datos = array(
                                'estado' => 0, 
                                'idNotificacion' => (int)$idNotificacion,
                                'mensaje' => 'No se envió la notificación', 
                                'resultadoEnvio' => 'Ok'
                            );
    ...
    // VER LOG LEIDA DE UNA NOTIFICACIÓN
    public function verLogLeida(Request $request, Response $response, $args){
        $tokenAcceso    = $request->getHeader('TokenAcceso');
        $idNotificacion = $args['idNotificacion'] ?? null;         
        ...
        if(isset($tokenAcceso[0])){
            if(verificarToken($tokenAcceso[0]) === true){
                ...
                if($error == 0){
                    // busco las notificaciones de este servicio
                    $con = 'EXEC notificacionLogLeida @idNotificacion = :idNotificacion';
                    try {
                        $db = getConeccionCAB();
                        $stmt = $db->prepare($con);
                        $stmt->bindParam(":idNotificacion", $idNotificacion);
                        ...
                        foreach($res as $notificacion){
                            $n = new \stdClass();
                            $n->fecha      = $notificacion->fecha;
                            $n->idDispositivo = $notificacion->idDispositivo;
                            $n->nombreDispositivo   = $notificacion->nombreDispositivo;
                            $n->idAplicacion       = $notificacion->idAplicacion;
                            $n->nombreAplicacion   = $notificacion->nombreAplicacion;
                            $n->dniUsuario   = $notificacion->dniUsuario;
                            $n->nombreUsuario   = $notificacion->nombreUsuario;
                            $n->apellidoUsuario   = $notificacion->apellidoUsuario;
